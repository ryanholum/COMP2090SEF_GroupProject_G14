...
class Basic_Properties:
    def __init__(self):
        pass
class Expression_Simplification:
    def __init__(self,expression):
        self.expression = expression

    # for check_associativity #
    def check_associativity(self, other):
        if not check_type([Element,Block],other):
            raise TypeError("Invalid type for check_associativity")
        self_elements = [element for element in self.expression if not isinstance(element,Operator)]
        self_operators = [element for element in self.expression if isinstance(element,Operator)]
        
        other_elements = [element for element in other.expression if not isinstance(element,Operator)]
        other_operators = [element for element in other.expression if isinstance(element,Operator)]
        

        if (self.coefficient == other.coefficient and
            self.power == other.power):
            if check_type(Add,self_operators+other_operators) and self_elements == other_elements:
                return other
            elif check_type(Mul,self_operators+other_operators) and self_elements == other_elements:
                return other
            elif check_type([Add,Mul],self_operators+other_operators) and self.expression == list(reversed(other.expression)):
                return other
            else:
                return self
        else:
